...
@pytest.mark.unit
def test_recover_parabola_non_zero_start_frame():
    """Regression: bundle_adjust used to IndexError when flight starts mid-clip."""
    K = np.array([[1500.0, 0, 960], [0, 1500.0, 540], [0, 0, 1]])
    look_world = np.array([0.0, 64.0, -30.0])
    look_world = look_world / np.linalg.norm(look_world)
    right_world = np.array([1.0, 0.0, 0.0])
    ...
    for i, p in enumerate(pts):
        cam = R @ p + t
        pix = K @ cam
        # CRITICAL: frame indices start at 50, not 0
        obs.append((50 + i, tuple(pix[:2] / pix[2])))
