...
- GPU-dependent validation (GVHMR, real WASB stage runs) cannot happen on this Mac. Mark those items "pending GPU-box run" — never let them pass silently.
...
For each verification item: the exact command, pass/fail, and the relevant output lines (failures verbatim). End with a verdict: APPROVED, APPROVED WITH PENDING GPU ITEMS (list them), or REJECTED (list blocking failures and the responsible files).
