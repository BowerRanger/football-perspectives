...
- `hmr_world.anchor_mode` / `hmr_world.contact.*` — root-translation anchoring strategy: `contact` (default) does per-foot ray-cast stance detection and pins the planted foot with a smooth correction delta over the legacy ankle-mid carrier (fixes root-translation sliding); `ankle_mid` is the legacy canonical-offset ankle-midpoint ray-cast, kept for bit-parity. `contact.*` tunes the stance hysteresis (`speed_enter_m_s`/`speed_exit_m_s`), span gating (`min_span_frames`/`max_pin_spread_m`), pixel-noise floor (`px_noise`), and the delta channel's clamp/decay (`max_correction_m`/`decay_s`). `refined_poses.foot_lock.*` — post-smoothing finale (runs last): re-pins each contact-detected stance span with two-bone leg IK (`ik_max_joint_delta_deg`/`max_residual_correction_m`/`edge_ease_frames` clamp and ease the correction, skipping rather than mangling a span that exceeds them) then a penetration guard that only ever raises `root_t.z` to keep the sole (`sole_clearance_m`) off the pitch plane; `refined_poses.ground_snap_target_z`/`ground_snap_max_distance` are the now-explicit (and contact-gated) ground-snap defaults consumed by the same pass.
