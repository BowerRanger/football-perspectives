...
- The static-C camera stage run takes ~9 min on gberch (429 frames) at the `line_extraction_static_rounds: 1` default — was ~15 min at 3 rounds. The remaining cost is dominated by the C-profile grid sweep (~340 s) plus the one detection pass and BA. The C-profile grid size / frame subsample are the next levers if it needs to be faster; a 15-min synchronous solve from the web dashboard should also eventually move to a background job.
