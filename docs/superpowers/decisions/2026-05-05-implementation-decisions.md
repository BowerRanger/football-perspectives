...
### D16: Fix GVHMR↔OpenCV camera-frame bridge in `smpl_pitch_transform`

**Context.** The 3D viewer rendered SMPL skeletons that were tipped over and partially below the pitch surface, even though `root_t.x/y` and the camera-track positions all looked sensible. Empirical decomposition (FK without per-joint thetas) showed the body's "up" direction landed in pitch +y instead of pitch +z — a consistent ~90° error across every frame.

**Root cause.** `smpl_root_in_pitch_frame` was composing GVHMR's prediction with the OpenCV extrinsic via:

```
root_R_pitch = R_w2c.T @ SMPL_TO_PITCH_STATIC @ root_R_cam
```

where `SMPL_TO_PITCH_STATIC = [[1,0,0],[0,0,-1],[0,1,0]]`. That matrix is a 90° rotation, but the conventions actually being bridged are GVHMR's **y-up camera frame** (body's "up" = camera +y) and OpenCV's **y-down camera frame**. The correct bridge is a 180° rotation around the camera +x axis — `[[1,0,0],[0,-1,0],[0,0,-1]]`. The two ts of conventions differ in both y and z signs, not just a 90° axis swap.

This was empirically confirmed by reverse-engineering `root_R_cam` from the saved `root_R_pitch` and the camera_track for every player: `root_R_cam @ (0, 1, 0)` was ≈ `(0, +1, 0)` for upright players (matching y-up camera, *not* y-down OpenCV).

**Decisions D8/D10 fixtures were also wrong but coincidentally passed** because they used `R_w2c = I` — which is unphysical (an OpenCV camera that's axis-aligned with a z-up world implies the camera frame is z-up, contradicting OpenCV's y-down). With both errors stacked the formula happened to give the right answer for that fixture; with a real broadcast `R_w2c` it gave the wrong answer.

**Fix.**

1. **`src/utils/smpl_pitch_transform.py`** — rename `SMPL_TO_PITCH_STATIC` → `GVHMR_TO_OPENCV_CAM` (with backwards-compat alias) and change the value to `[[1,0,0],[0,-1,0],[0,0,-1]]`. Updated docstring to document the actual conventions being bridged.

2. **`src/stages/hmr_world.py`** — `_FOOT_IN_ROOT` was `(0, 0, -0.95)`, expressed in body-local coordinates that the wrong matrix had coincidentally aligned with z-up. With the fix, `root_R_pitch` correctly takes SMPL canonical (y-up) directly to pitch z-up, so the body-local "below" is `−y`: `_FOOT_IN_ROOT` = `(0, -0.95, 0)`.

3. **`src/web/static/viewer.html`** — `SMPL_J_REST` reverted to standard SMPL canonical y-up positions (head at +y, feet at −y).

4. **`tests/test_smpl_pitch_transform.py`** — both regression tests rewritten with a physically-realistic broadcast `R_w2c = [[1,0,0],[0,0,-1],[0,1,0]]` (the convention the camera-track stage actually emits). The yaw test was multiplying its rotation on the **left** of `R_w2c` (a camera-frame *roll*) instead of the **right** (a world-frame pan); fixed.

**Verification.** Re-applied the fixed formula in-place to all 23 stored players' `root_R_cam` (recovered from the broken `root_R_pitch`):

| metric                          | before fix         | after fix          |
|---------------------------------|--------------------|--------------------|
| `R @ (0,1,0)`'s pitch z (avg)   | −0.05 to −0.29     | +0.91 to +0.99     |
| foot offset projects to pitch z | inconsistent       | −0.91 m to −0.94 m |

**Action required.** The existing `output/hmr_world/*_smpl_world.npz` files were computed with the broken formula. They need to be regenerated by re-running `hmr_world` (the dashboard's *Re-run Stage* button wipes the directory; the per-player cache would otherwise serve stale data).

**Out of scope (still deferred):**
- D11 anchor-editor projection convention.
- Anchor smoothing.
- `pytest --cov` re-instatement.
- Multi-shot (or single-shot kp2d) refinement of GVHMR's per-frame
  SMPL output. A SMPLify-style stage was implemented and reverted —
  the recovered pose did not visibly improve over GVHMR's prediction
  for the broadcast clip we were working from. Revisiting later as
