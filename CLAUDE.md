...
- `hmr_world.anchor_mode` / `hmr_world.contact.*` — root-translation anchoring strategy: `contact` (default) does per-foot ray-cast stance detection and pins the planted foot with a smooth correction delta over the legacy ankle-mid carrier (fixes root-translation sliding); `ankle_mid` is the legacy canonical-offset ankle-midpoint ray-cast, kept for bit-parity. `contact.*` tunes the stance hysteresis (`speed_enter_m_s`/`speed_exit_m_s`), span gating (`min_span_frames`/`max_pin_spread_m`), pixel-noise floor (`px_noise`), and the delta channel's clamp/decay (`max_correction_m`/`decay_s`). Wave 5 added three per-span quality floors — `min_span_ankle_conf`, `min_span_quality`, `max_pin_spread_over_noise` (spread relative to the local pixel-noise floor) — all `null` (disabled) by default on gberch: a per-span sweep found none of the three separates the seven originally-failing players from the fifteen passing ones at the raw-ray-cast level (both groups span the same confidence/quality/spread range), so a binding threshold cuts good players' spans (e.g. collapsing P001/P017) without catching the actual offenders. Re-sweep before enabling on a clip where kp2d noise genuinely correlates with player size/distance. `refined_poses.foot_lock.*` — post-smoothing finale (runs last): re-pins each contact-detected stance span with two-bone leg IK (`ik_max_joint_delta_deg`/`max_residual_correction_m`/`edge_ease_frames` clamp and ease the correction, skipping rather than mangling a span that exceeds them) then a penetration guard that only ever raises `root_t.z` to keep the sole (`sole_clearance_m`) off the pitch plane; `refined_poses.ground_snap_target_z`/`ground_snap_max_distance` are the now-explicit (and contact-gated) ground-snap defaults consumed by the same pass. Wave 5's "honest unlockable-span handling" adds `resolved_pin_err_m`/`resolved_max_step_m`: a span that lands within `skip_pin_err_m` (so it isn't skipped) but leaves more residual than these tighter bounds (distance-from-pin and worst consecutive-frame step, respectively — the latter catches a clamp-limited outlier frame that passes the pin-distance check yet still reads as a skate spike) is treated like a skip for pose output AND excluded from the `{pid}_resolved_contacts.json` sidecar the stage persists (`src/schemas/foot_contacts.py`, `anchor_mode="resolved"`) — the verified/effective contact set `scripts/eval_foot_quality.py` prefers over the raw hmr_world sidecar when scoring a refined-stage track. This is the lever that actually fixed gberch's seven failing players (their raw ray-cast quality looks fine; the noise only surfaces after refined_poses' cleanup/jitter/consensus passes touch the carrier).
