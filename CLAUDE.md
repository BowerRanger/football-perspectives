...
- `ball.context_prior.*` — plausibility score modifier (never a hard gate) from prior-stage outputs: pitch-polygon, player-proximity, and pixel-static-while-panning signals multiply detector confidence before the accept threshold, suppressing stable wrong-object detections (frame-top blobs, scoreboard). Penalties stack multiplicatively across signals (floored by `min_factor`), so clips with many legitimately player-distant ball moments (long balls, clearances) can see real detections demoted too — validate `detection_coverage` per clip, not just gberch.
- `ball.touch_attribution.*` — post-pass that relabels each touch event's (player, bone) to the ray-closest joint over a small window around the event frame; strictly count-preserving (never adds or removes events, only relabels — same length/order as input)
