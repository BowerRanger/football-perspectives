"""Plausibility checks for fitted ball trajectories.

Every fitted parabola or Magnus segment is sampled at several time
points and rejected unless all samples stay within physical bounds
(pitch envelope + speed + height). Used by the piecewise solver as the
safety net on open-span fits, where no anchor nodes constrain the arc.
"""
